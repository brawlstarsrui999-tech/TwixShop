...
async def cb_admin_product(callback: CallbackQuery, config: Config):
    ...
    text = (
        f"📦 <b>{product['name']}</b>\n"
        f"ID: <code>{product['id']}</code> | {status}\n\n"
...
async def cb_add_product_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "<i>(Например: VIP, PREMIUM, God Mode)</i>",
...
async def process_product_name(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"✅ Название: <b>{name}</b>\n\n"
        f"Шаг 2/6 — Введите <b>описание</b> товара:\n"
        f"<i>(Что входит в привилегию, какие плюшки получит игрок)</i>",
        reply_markup=cancel_kb(),
        parse_mode="HTML",
    )
...
async def process_product_description(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"Шаг 3/6 — Введите <b>цену</b> в рублях:\n"
        f"<i>(Только число, например: 149 или 99.5)</i>",
        reply_markup=cancel_kb(),
        parse_mode="HTML",
    )
...
async def process_product_price(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"✅ Цена: <b>{price}₽</b>\n\n"
        f"Шаг 4/6 — Введите <b>тег/категорию</b> товара:\n"
        f"<i>(Например: VIP, Донат, Сезонные. Отправьте '-' чтобы пропустить)</i>",
        reply_markup=cancel_kb(),
        parse_mode="HTML",
    )
...
async def process_product_tag(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"Шаг 5/6 — Отправьте <b>фотографию</b> товара:\n"
        f"<i>(Отправьте изображение или '-' чтобы пропустить)</i>",
        reply_markup=cancel_kb(),
        parse_mode="HTML",
    )
...
async def process_product_photo(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"✅ Фото сохранено.\n\n"
        f"Шаг 6/6 — Введите <b>название группы LuckPerms</b>:\n"
        f"<i>(Именно то имя группы, которое используется в /lp group list)\n"
        f"Например: vip, premium, elite, god</i>",
        reply_markup=cancel_kb(),
        parse_mode="HTML",
    )
...
async def process_product_photo_skip(message: Message, state: FSMContext):
    if message.text.strip() == "-":
        ...
        await message.answer(
            f"✅ Фото пропущено.\n\n"
            f"Шаг 6/6 — Введите <b>название группы LuckPerms</b>:\n"
            f"<i>(Например: vip, premium, elite, god)</i>",
            reply_markup=cancel_kb(),
            parse_mode="HTML",
        )
    else:
...
async def cb_edit_field(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"Введите новое <b>{field_label}</b>:",
        reply_markup=cancel_kb(),
        parse_mode="HTML",
    )
    await callback.answer()
...
async def process_edit_value(message: Message, state: FSMContext, config: Config):
    ...
    if field == "photo":
        ...
        if not re.match(r'^[a-z0-9_-]{1,32}$', lp_group):
            await message.answer("❌ Некорректное имя LP группы. Только строчные буквы, цифры, '-', '_':")
            return
