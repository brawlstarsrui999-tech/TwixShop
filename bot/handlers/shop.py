...
async def cb_product(callback: CallbackQuery, config: Config):
    ...
    text = (
        ...
        f"🏷 Категория: {product['tag'] or 'Без категории'}\n"
...
async def cb_buy(callback: CallbackQuery, state: FSMContext, config: Config):
    ...
    price_str = f"{product['price']:.0f}" if product['price'] == int(product['price']) else f"{product['price']:.2f}"
    text = (
        f"💳 Покупка: <b>{product['name']}</b>\n"
        f"💰 Цена: {price_str}₽\n\n"
        f"✏️ Введите ваш ник в Minecraft (точно, с учётом регистра):"
    )
...
async def cb_confirm_buy(callback: CallbackQuery, state: FSMContext, config: Config):
    ...
    if success:
        ...
        text = (
            ...
            f"👤 Ник: <code>{nickname}</code>\n"
            f"🔑 Группа: <code>{product['lp_group']}</code>\n\n"
            f"❌ Не удалось подключиться к серверу или выдать привилегию.\n"
        ...
        for admin_id in config.bot.admin_ids:
            try:
                await callback.bot.send_message(
                    ...
                    f"Выдайте вручную:\n<code>lp user {nickname} parent add {product['lp_group']}</code>",
                    parse_mode="HTML",
                )
            except Exception as e:
                logger.warning(f"Не удалось уведомить админа {admin_id}: {e}")
    ...
    try:
        await processing_msg.edit_text(text, reply_markup=back_to_main_kb(), parse_mode="HTML")
    except Exception:
