...
Таблицы:
  ...
  - pending_orders — заказы, ожидающие ввода ника
"""
...
async def get_all_products(db_path: str, active_only: bool = True) -> list[dict]:
    async with aiosqlite.connect(db_path) as db:
...
async def get_products_by_tag(db_path: str, tag: str) -> list[dict]:
    async with aiosqlite.connect(db_path) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
...
async def get_user_orders(db_path: str, user_id: int) -> list[dict]:
    async with aiosqlite.connect(db_path) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
...
async def get_all_orders(db_path: str, limit: int = 50) -> list[dict]:
    async with aiosqlite.connect(db_path) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
